...
```bash
docker run --rm -v ${PWD}/docs:/docs $(docker build -q -t sphinx -f docs/docker-compose.yml ./docs) make singlehtml
```
...
```bash
docker run --rm -it -v ${PWD}/docs:/docs -p 8000:8000 $(docker build -q -t sphinx -f docs/docker-compose.yml ./docs)
```
